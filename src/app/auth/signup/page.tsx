...
const SignUpPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Only include emailAddress and password in the initial sign-up request
            await signUp.create({
                emailAddress: email,
                password,
            });
    ...
    const handleVerify = async (e: React.FormEvent) => {
        ...
        if (!isLoaded || !signUp) {
            toast.error("Sign-up process not initialized. Please try again.");
            return;
        }
        ...
        <div className="flex flex-col justify-center items-center bg-gradient-to-tr from-blue-600 via-indigo-500 to-purple-700 p-4 min-h-screen">
            <div className="space-y-8 bg-white bg-opacity-10 shadow-2xl backdrop-blur-xl p-8 rounded-3xl w-full max-w-md transform hover:scale-105 transition-all duration-300">
                <div className="flex flex-col gap-4 text-center">
                    ...
                    <p className="text-lg text-white text-opacity-80">
                        Create an account to start using <span className="bg-clip-text bg-gradient-to-r from-pink-800 to-blue-800 text-transparent"> Swasthya Darpan</span>       
                    </p>
                </div>
